...
const CCTV = () => {
  ...
  const setUpdateSpecialPlate = async(update: RealTimeLprData) => {
    try {
      if (update.is_special_plate !== 1) return
        ...
        <div className="w-full h-full pl-[10px] pr-[20px] pt-[15px] pb-2 border-[1px] border-dodgerBlue rounded-[10px]">
          <div className={`w-full ${selectedScreenValue > 1 ? "grid grid-cols-2 lt1535:grid-cols-1" : "grid grid-cols-1 h-[98%]"} h-full gap-y-1 gap-x-5 overflow-y-auto`}>
          {
            cameraDetailSettingData.slice(0, selectedScreenValue).map((live, index) => (
              <div key={live.camera_name} id={`CCTV${index + 1}`} className="flex flex-col relative">
                {/* CCTV Stream */}
                <div
                ...
                <div className="absolute top-1 left-[160px] text-[15px] text-white max-w-[200px]">
                  {
                    (() => {
                      const checkpointName = `${t('text.checkpoint')}: ${activeStreamUrls[index] ? activeStreamUrls[index].name : live.camera_name}`
                      return (
                                ...
                                <div className="flex flex-col h-[114px] w-full text-center bg-tuna">
                                  <p className="text-white text-[24px] font-medium">
                                    {`
                                      ...
                                      `${lprData.plate_prefix}-${lprData.plate_number}` : 
                                      `${lprData.plate_prefix} ${lprData.plate_number}`} 
                                      ${i18n.language === "th" ? lprData.region_name_th : lprData.region_name_en}
                                    `}
                                  </p>
                                  ...
                                  <div className="grid grid-cols-2 text-white text-[18px] font-light p-2">
                                    <div className='border-r-[1px] border-gainsboro'>
                                      <p className='truncate' title={`${reformatString(lprData.vehicle_make)} ${reformatString(lprData.vehicle_model)}`}>{reformatString(lprData.vehicle_make)} {reformatString(lprData.vehicle_model)}</p>
                            ...
                            {streamLPRData && selectedScreenValue === 3 ? (
                              <div className="flex flex-1 flex-col h-full items-center justify-center">
                                ...
                                <div className="flex flex-col h-[122px] w-full text-center bg-tuna">
                                  <p className="text-white text-[24px] font-medium">
                                    {`
                                      ${isNumber(streamLPRData?.plate_prefix) && isNumber(streamLPRData?.plate_number) ? 
                                      `${streamLPRData?.plate_prefix}-${streamLPRData?.plate_number}` : 
                                      `${streamLPRData?.plate_prefix} ${streamLPRData?.plate_number}`} 
                                      ${i18n.language === "th" ? lprData.region_name_th : lprData.region_name_en}
                                    `}
                                  </p>
                                  ...
                                  <div className="grid grid-cols-2 text-white text-[18px] font-light p-2">
                                    <div className='border-r-[1px] border-gainsboro'>
                                      <p className='truncate' title={`${reformatString(streamLPRData.vehicle_make)} ${reformatString(streamLPRData.vehicle_model)}`}>{reformatString(streamLPRData.vehicle_make)} {reformatString(streamLPRData.vehicle_model)}</p>
