...
const TextUpload: React.FC<TextUploadProps> = ({setTextsDataList, textsDataList}) => {
  ...
  const [selectedCheckpointIds, setSelectedCheckpointIds] = useState<{value: any, label: string}[]>([]);
  ...
  // Options
  const [checkpointsOption, setCheckpointsOption] = useState<{value: any, label: string}[]>([]);
  ...
  useEffect(() => {
    if (sliceDropdown.checkpoints && sliceDropdown.checkpoints.data) {
      ...
      setCheckpointsOption([{ label: t('dropdown.all'), value: "0" }, {label: t('dropdown.center'), value: "center"}, ...options]);
    }
  }, [sliceDropdown.checkpoints, i18n.language, i18n.isInitialized]);

  useEffect(() => {
    if (sliceDropdown.checkpoints && sliceDropdown.checkpoints.data.length > 0) {
      const hasAll = selectedCheckpointObjects.some((v) => v.value === "0");
      const newCheckpointList = hasAll ? checkpointsOption.filter(c => c.value !== "0") : checkpointsOption.filter(c => selectedCheckpointObjects.map(sc => sc.value).includes(c.value));
      setSelectedCheckpointIds(newCheckpointList);
    }
  }, [selectedCheckpointObjects, sliceDropdown.checkpoints, checkpointsOption])
  ...
  const handleCheckpointChange = (ids: string[]) => {
    ...
    const selectedObjects = checkpointsOption.filter(c => newIds.includes(c.value));
    setSelectedCheckpointObjects(selectedObjects);

    const hasAll = selectedObjects.some((v) => v.value === "0");
    
    const finalCheckpoints = hasAll
      ? checkpointsOption.filter(c => c.value !== "0")
      : selectedObjects;

    setSelectedCheckpointIds(finalCheckpoints);
  ...
  const handleDeleteChip = (rowIndex: number, checkpointValue: any) => {
    setTextsData(prev =>
      prev.map((row, index) =>
        index === rowIndex
          ? {
              ...row,
              checkpoints: row.checkpoints?.filter(
                cp => cp.value !== checkpointValue
              ),
            }
          : row
      )
    ...
    <div id='text-upload'>
      ...
      <div className='flex flex-col h-full'>
        <div className='flex items-center justify-between'>
          <div className='flex items-center w-[900px] gap-2'>
            <p className='text-[15px] text-white'>{t('component.select-checkpoint')}</p>
            <div className='w-[30vw] items-center justify-center'>
              <MultiSelectCameras 
                limitTags={2} 
                selectedValues={selectedCheckpointObjects}
                options={checkpointsOption} 
                onChange={handleCheckpointChange}
                placeHolder={t('placeholder.select-checkpoint')}
              />
            </div>
          </div>
          ...
          <Button
            ...
            sx={{ 
              ...
              minWidth: t('button.import-excel-width'), 
              height: "30px",
            ...
            <Table stickyHeader>
              ...
              >
                <TableRow>
                  ...
                  <TableCell sx={{ textAlign: "center" }}>{t('table.column.destination-checkpoint')}</TableCell>
                        ...
                        <Stack direction="column" spacing={1}>
                          {data.checkpoints?.map((checkpoint) => (
                            <Chip
                              ...
                              variant="outlined"
                              onDelete={() =>
                                handleDeleteChip(index, checkpoint.value)
                              }
                              sx={{
                                "&.MuiButtonBase-root": {
