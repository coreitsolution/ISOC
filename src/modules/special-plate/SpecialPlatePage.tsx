...
// Constant
import { SPECIAL_ROW_PER_PAGES } from "../../constants/dropdown";
...
const SpecialPlatePage: React.FC<SpecialPlateProps> = ({}) => {
  ...
  const [rowsPerPage, setRowsPerPage] = useState(SPECIAL_ROW_PER_PAGES[SPECIAL_ROW_PER_PAGES.length - 1]);
  const [rowsPerPageOptions] = useState(SPECIAL_ROW_PER_PAGES);
  ...
  const fetchSpecialPlates = async (page: number, limit: number, filter?: string) => {
    ...
    try {
      setIsLoading(true);
      const response = await fetchClient<SpecialPlateResponse>(combineURL(API_URL, "/special-plates/get"), {
        method: "GET",
  ...
  const downloadFiles = async (filename: string, uid: string) => {
    ...
    const timeoutId = setTimeout(() => controller.abort(), 10000);
    try {
      const response = await fetchClient<ZipDownloadResponse>(combineURL(API_URL, "/special-plates/get-documents-url"), {
        method: "GET",
        signal: controller.signal,
        queryParams: {
          uid: `${uid}`
  ...
  const handleDelete = async (uid: string) => {
    try {
      ...
      const imageResponse = await fetchClient<SpecialPlateFilesResponse>(combineURL(API_URL, `/special-plate-images/get`), {
        method: "GET",
        queryParams: {
      ...
      if (imageResponse.data.length > 0) {
        ...
        await fetchClient<SpecialPlateResponse>(combineURL(API_URL, `/special-plate-images/delete`), {
          method: "DELETE",
          queryParams: {
            uids: imagesIdList.join(',')
          },
        })
      }

      const fileResponse = await fetchClient<SpecialPlateFilesResponse>(combineURL(API_URL, `/special-plate-files/get`), {
        method: "GET",
        queryParams: {
          filter: `special_plate_uid=${uid}`
        }
  ...
  const handleSearch = async (searchFilter: FormData) => {
    ...
    if (searchFilter.status !== 2) {
      filterParts.push(`active=${searchFilter.status === 1 ? true : false}`);
    }
  ...
  return (
    <div id='special-plate' className={`main-content ${isOpen ? "pl-[130px]" : "pl-2.5"} pr-2.5 transition-all duration-500`}>
      ...
      <div className="grid grid-cols-[1fr_270px] gap-x-4">
        <div>
          {/* Header */}
          <Typography variant="h5" color="white" className="font-bold">{t('screen.special-plate')}</Typography>
          {/* Body */}
                ...
                <Table sx={{ minWidth: 650, backgroundColor: "#48494B"}}>
                  <TableHead>
                    ...
                    >
                      ...
                      <TableCell align="center" sx={{ color: "#FFFFFF", width: "4%" }}>{t('table.column.edit-delete')}</TableCell>
                    </TableRow>
                  </TableHead>
                  <TableBody>
                    {
