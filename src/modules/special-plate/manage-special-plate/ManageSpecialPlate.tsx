...
const ManageSpecialPlate: React.FC<ManageSpecialPlateProps> = ({open, onClose, selectedRow}) => {
  ...
  const handleDeleteImage = useCallback(async (position: number, fileData: FileData) => {
    try {
      ...
      await fetchClient<SpecialPlateResponse>(combineURL(API_URL, `/upload/remove`), {
        method: "POST",
        body,
      })
    }
  ...
  const handleImageUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetchClient<SpecialPlateFilesResponse>(combineURL(API_URL, "/upload/"), {
        method: "POST",
        isFormData: true,
        body: formData,
      })
  ...
  const updateSpecialPlate = async (data: any) => {
    try {
      ...
      const response = await fetchClient<SpecialPlateResponse>(combineURL(API_URL, "/special-plates/update"), {
        method: "PATCH",
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      if (isImageChanged) {
        ...
        if (removed.length > 0) {
          await fetchClient<SpecialPlateResponse>(combineURL(API_URL, `/special-plate-images/delete`), {
            ...
            queryParams: {
              uids: removed.map((image) => image.uid).join(",")
            }
          })
        }
        ...
        if (added.length > 0) {
          await Promise.all(
            added.map(async (image) => {
              ...
              await fetchClient<SpecialPlateResponse>(combineURL(API_URL, `/special-plate-images/create`), {
                method: "POST",
                headers: {
                  'Content-Type': 'application/json'
                },
      ...
      if (isFileChanged) {
        ...
        if (removed.length > 0) {
          await fetchClient<SpecialPlateResponse>(combineURL(API_URL, `/special-plate-files/delete`), {
            ...
            queryParams: {
              uids: removed.map((file) => file.uid).join(",")
            }
          })
        }
        ...
        if (added.length > 0) {
          await Promise.all(
            added.map(async (file) => {
              ...
              await fetchClient<SpecialPlateResponse>(combineURL(API_URL, `/special-plate-files/create`), {
                method: "POST",
                headers: {
                  'Content-Type': 'application/json'
                },
  ...
  const deleteImportData = async (list: SpecialPlateFileData[]) => {
    await Promise.all(
      list.map(async (data) => {
        ...
        await fetchClient<SpecialPlateFilesResponse>(combineURL(API_URL, `/upload/remove`), {
          method: "POST",
          body,
        })
      })
  ...
  return (
    <Dialog id='manage-special-plate' open={open} maxWidth="xl" fullWidth>
      <DialogTitle className='bg-black'>
        {/* Header */}
        <div>
          <Typography variant="h5" color="white" className="font-bold">{t('screen.manage-special-plate')}</Typography>
        </div>
      </DialogTitle>
